module InPlaceMacrosHelper
  ...
  # Renders the value of the and method with in-place editing capabilities.
  ...
  def in_place_collection_editor(field_id, collection=[], options = {})
  ...
  # Renders the value of the and method with in-place editing capabilities.
  def in_place_editor_select(object, method, collection=[], tag_options = {}, in_place_editor_options = {})
    if object.kind_of?(::ActiveRecord::Base)
      name = object.class.to_s.underscore
      tag = ::ActionView::Helpers::InstanceTag.new(name, method, self, object)
    else
    ...
    tag_options = {:tag => "span", :id => "#{name}_#{method}_#{tag.object.id}_in_place_collection_editor",
                   :class => "in_place_editor_select"}.merge!(tag_options)

    in_place_editor_options[:url] = in_place_editor_options[:url] ||
                                    url_for(:action => "set_#{name}_#{method}",
                                            :id => tag.object.id)

    select = tag.to_content_tag(tag_options.delete(:tag), tag_options).
              # strip tags
              gsub(%r{(<span.*?>)(.*?)(</span>)}m){
                "#{$1}#{strip_tags($2)}#{$3}"
              }
    match = select.match(%r{(<span.*?>)(.*?)(</span>)}m)
    if match[2].blank?
      select = "#{match[1]}#{in_place_editor_options[:text_if_blank]}#{match[3]}"
    end

    select + in_place_collection_editor(tag_options[:id], collection, in_place_editor_options)
  end
end
